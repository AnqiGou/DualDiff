...
def get_ld(ls1, ls2):
    ...
    str_ls1 = ''.join(map(str, ls1)) 
    str_ls2 = ''.join(map(str, ls2)) 

    edit_dist = Levenshtein.distance(str_ls1, str_ls2)
    return 1 - edit_dist / (max(len(str_ls1), len(str_ls2)) + 1e-5)
...
def main():
    predictor = pipeline(Tasks.ocr_recognition, model='/path/to/model')
